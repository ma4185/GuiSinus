...
% Authoren: Nora Jürgens, Hanna Möhle, Maren Osterloh
...
scale = 30/(2*amp);
idx=1;
time=(0:0.01:5); % Zeitvektor zur Erstellung des Linienplots
...
% Verknüpfung der Schallquellen in der Matrix
Z = exp(I*freq*R1)./R1+ exp(I*freq*R2)./R2;


%% Buttons

% Erstellen von Buttons zur Einstellung der Frequenz, Amplitude, der
% Anzahl der Schallquellen und zum Starten und Stoppen der Animation
...
% der Graphik geändert
    ...
    function number_source()
        ...
        switch auswahl_schall{wahl_schall}
            case '2 sound sources'
                Z = exp(I*freq*R1)./R1 + exp(I*freq*R2)./R2;
                
            case '3 sound sources'
                Z = exp(I*freq*R1)./R1 + exp(I*freq*R2)./R2+ exp(I*freq*R3)./R3;
                
            case '4 sound sources'
